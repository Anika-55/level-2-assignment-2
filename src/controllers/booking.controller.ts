...
export const createBooking = async (req: AuthRequest, res: Response) => {
  ...
  try {
    // 1️ Check if vehicle exists and is available
    ...
    if (!vehicle) {
      return res
        .status(404)
        .json({ success: false, message: "Vehicle not found or not available" });
    }

    // 2️ Check overlapping bookings
    ...
    if (overlapQuery.rows.length > 0) {
      return res
        .status(400)
        .json({ success: false, message: "Vehicle already booked for selected dates" });
    }
    ...
    // 5️ Update vehicle availability
    await pool.query(
...
export const getBookings = async (req: AuthRequest, res: Response) => {
  try {
    let query = `
      SELECT b.*, u.name AS customer_name, v.vehicle_name, v.type, v.daily_rent_price
      FROM bookings b
...
export const updateBooking = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    // Customer can only cancel own booking before rental starts
    if (req.user.role === "customer") {
      if (status !== "cancelled") {
    ...
    // Update vehicle availability when booking is cancelled or returned
