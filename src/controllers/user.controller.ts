...
// DELETE user (Admin only, only if no active rentals)
export const deleteUser = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const rentals = await pool.query(
      `SELECT * FROM rentals WHERE user_id=$1 AND end_date >= CURRENT_DATE`,
      [userId]
    );
    ...
    const result = await pool.query(`DELETE FROM users WHERE id=$1 RETURNING id,name,email,role`, [userId]);
